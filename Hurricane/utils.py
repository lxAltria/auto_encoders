...
def resize_data(data, block_size):
	...
	return resized_data.reshape([num * n * n, block_size, block_size])
...
def synthesize_data(data, block_size):
	...
	count_matrix = np.ones(data_shape[-2], data_shape[-1], dtype=np.int8)
	for j in range(n - 1):
		for k in range(n - 1):
			count_matrix[pos[j+1]:pos[k] + block_size, pos[k+1]:pos[k] + block_size] = count_matrix[pos[j+1]:pos[k] + block_size, pos[k+1]:pos[k] + block_size] + 1
	...
	for i in range(num):
		...
		data_syn[i] = data_syn[i] / count_matrix
	return data_syn
